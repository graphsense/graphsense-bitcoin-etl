...
class BtcStreamerAdapter:
    def __init__(
        self, bitcoin_rpc, chain=Chain.BITCOIN, batch_size=2, max_workers=5
    ):
...
def hex_to_bytearray(hex_str: str) -> Optional[bytearray]:
    ...
    return bytearray.fromhex(hex_str) if hex_str is not None else None
...
def parse_script(s: str):
    ...
    if script.type == "p2pk":
        try:
            return [
                str(P2pkhAddress(script.pubkey.hash(), mainnet=True))
            ], script.type
        except:
            raise ValueError(
    ...
    if script.type == "p2pkh":
        return [
            str(P2pkhAddress(script.pubkeyhash, mainnet=True))
        ], script.type
...
def ingest_transaction_lookups(
    ...
    res = [
        dict((k, tx[k]) for k in ("tx_prefix", "tx_hash", "tx_id"))
        for tx in txs
    ]
...
def create_parser():
    parser = ArgumentParser(
        description="bitcoin-etl ingest into Apache Cassandra",
...
def main() -> None:
    ...
    thread_proxy = ThreadLocalProxy(lambda: BitcoinRpc(args.provider_uri))
    btc_adapter = BtcStreamerAdapter(thread_proxy, batch_size=args.batch_size)

    resolver = OutputResolver(session, args.keyspace, TX_BUCKET_SIZE)
    ...
    if args.start_block is None and start_block > end_block:
        print(
    ...
    print(
        f"[{datetime.now()}] Ingesting block range {start_block:,}:{end_block:,} into Cassandra nodes {args.db_nodes}"
    )
    ...
    for block_id in range(start_block, end_block + 1, args.batch_size):
        ...
        enrich_txs(
            txs, resolver
        )  # until bitcoin-etl progresses with https://github.com/blockchain-etl/bitcoin-etl/issues/43
        ...
        ingest_transaction_lookups(
            txs, session, prep_stmt["transaction_by_tx_prefix"]
        )
    ...
    cql_str = """INSERT INTO summary_statistics
                 ...
                 VALUES (%s, %s, %s, %s)"""
    session.execute(
        cql_str, (args.keyspace, timestamp, total_blocks, total_txs)
    )
